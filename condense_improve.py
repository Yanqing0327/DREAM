...
def matchloss(args, img_real, img_syn, lab_real, lab_syn, model):
    ...
    if args.match == 'feat':
        ...
        loss = add_loss(loss, dist(feature_real.mean(0), feature_syn.mean(0), method=args.metric))
...
def condense(args, logger, device='cuda'):
        ...
        for c in range(synset.nclass):
            if args.f2_init=='random':
                ...
                if args.dataset == 'tinyimagenet':
                    query_idxs= strategy.query_tiny(synset.ipc * synset.factor**2)
                else:
    ...
    torch.save(
                ...
                os.path.join(args.save_dir, f'data{0}.pt'))
    ...
    print(synset.best1,synset.best2)
    # Data distillation
        ...
        for ot in range(args.inner_loop):
            ...
            for c in range(nclass):
                if ot % args.interval == 0:
                    ...
                    if args.dataset == 'tinyimagenet':
                        query_idxs= strategy.query_tiny(args.batch_real)
                    else:
                        query_idxs= strategy.query_match(args.batch_real,args.subsample)
    ...
    logger(
            f'Best_{args.net_type},  Best_resnet: {synset.best1:.1f} {synset.best2:.1f}\n')
